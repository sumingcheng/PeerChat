...
const Message: React.FC<MessageProps> = ({ message }) => {
  ...
  // 检查发送者是否是主持人
  const isHost =
    message.isHost || // 1. 直接使用消息中的isHost字段
    (currentChat &&
      currentChat.isGroup &&
  ...
  // 检查当前用户是否是主持人
  const currentUserIsHost = currentChat && currentChat.isGroup && (currentChat as GroupChat).isHost;
  ...
  const getStatusIndicator = () => {
    ...
    switch (messageStatus) {
      ...
      case 'sent':
        return <span className="ml-2 text-gray-400">已发送</span>;
      case 'delivered':
        return <span className="ml-2 text-blue-500">已送达</span>;
      case 'read':
        return <span className="ml-2 text-green-500">已读</span>;
      case 'error':
        return (
  ...
  // 调试信息
  console.log(
    `消息: ${message.content}, 发送者: ${message.sender}, roomId: ${message.roomId}, 是否主持人: ${isHost}`
  );

  return (
    <div className={`flex ${isOwn ? 'flex-row-reverse' : 'flex-row'} mb-4`}>
      <Avatar alt={message.senderName || '用户'} size="sm" className={isOwn ? 'ml-3' : 'mr-3'} />
